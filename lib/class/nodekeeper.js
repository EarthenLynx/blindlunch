...
class Nodekeeper {
  ...
  create(length) {
    ...
    if (process.env.NODE_ENV === "development") log(3, `Creating node.env global with length ${aLength}. Value: ${random} `)
    return random;
  }
  ...
  // FIXME: Sometimes, this compare function would not work properly and throw
  // an error even though the key does exist.
  async compare(key, value) {
    console.log(process.env[key]);
    if(process.env[key]) {
      return await process.env[key] === value
    } else {
